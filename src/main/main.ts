...
import { importSeedOnce, type SeedImportResult } from './seed';
...
function registerHandlers(): void {
  ...
  ipcMain.handle('seed:import', () => importSeedData(true));
  ipcMain.handle('deadlines:list', (_e, filter) => database.listDeadlines(filter ?? {})); ipcMain.handle('deadlines:create', (_e, input) => database.createDeadline(input)); ipcMain.handle('deadlines:update', (_e, id, input) => database.updateDeadline(id, input)); ipcMain.handle('deadlines:delete', (_e, id) => database.deleteDeadline(id)); ipcMain.handle('deadlines:set-completed', (_e, id, completed) => database.setDeadlineCompleted(id, completed));
